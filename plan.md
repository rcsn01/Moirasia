...
**Q6 — Security posture?**
➡️ Shared module always: `mkdir(parent, { recursive: true, mode: 0o700 })`; new lock candidates use `mkdir(candidate, { mode: 0o700 })` + `chmod(candidate, 0o700)` and inherit that mode through the initial atomic `rename`; owner and recovery candidate files use `mode: 0o600`. Owner records are published atomically, and recovery runs `chmod(lockPath, 0o700)` while its marker is held before replacing stale ownership in place. This preserves Shout/Bonded's posture and hardens legacy Vox directories.
