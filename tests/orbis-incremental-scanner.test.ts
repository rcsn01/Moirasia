...
import { FSEVENT_FLAGS, type ChangeEvent } from '../packages/feature-orbis/src/main/change-journal'
import { planDirtyScopes } from '../packages/feature-orbis/src/main/incremental-scanner'
...
describe('Orbis dirty-scope planning', () => {
  ...
  it('falls back for an unmatched rename', async () => {
    const directory = await fixture()
    const target = join(directory, 'target')
    ...
    expect(plan).toEqual({ kind: 'full', reason: 'rename-ambiguous' })
  })
})
