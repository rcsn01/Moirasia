...
describe('feature catalog', () => {
  it('pins the catalog order that menu accelerators and index-dependent consumers rely on', () => {
    expect(FEATURE_IDS).toEqual(['amove', 'vox', 'exithibition', 'bonded', 'orbis'])
    expect(featureCatalog.entries.map((entry) => entry.id)).toEqual(FEATURE_IDS)
  })

  it('accepts exactly the five feature ids and rejects lookalikes', () => {
    for (const id of FEATURE_IDS) expect(isFeatureId(id)).toBe(true)
    expect(featureCatalog.isId('vox')).toBe(true)
  ...
  it('keeps the requirements transcribed from the former defaultRequirements switch', () => {
    expect(featureCatalog.get('vox').requirements.suite).toEqual({ preloads: ['overlay'], renderers: ['overlay'], native: ['executable'], dataDirectory: true })
  ...
  it('matches bundle ids and executable names to the standalone bundles', () => {
    ...
    expect(featureCatalog.get('vox').bundleId).toBe('com.moirasia.vox')
    ...
    expect(featureCatalog.entries.map((entry) => entry.executableName)).toEqual(['Amove', 'Vox', 'Exithibition', 'Bonded', 'Orbis'])
  })
