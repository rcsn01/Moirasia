...
const tag = `v${version}`;
const dmg = join(root, "release", `Moirasia-${version}-arm64.dmg`);
...
if (args.has("--dry-run")) {
  ...
  console.log("Actual flow: preflight → package:mac → tag → atomic push → GitHub Release upload");
  process.exit(0);
}
...
run("pnpm", ["package:mac"], "macOS packaging failed.");
...
run("git", ["tag", "-a", tag, "-m", `Moirasia ${version}`], `Unable to create tag ${tag}.`);
...
const release = spawnSync(
  "gh",
  ["release", "create", tag, dmg, checksum, "--verify-tag", "--generate-notes", "--title", `Moirasia ${version}`],
  { cwd: root, stdio: "inherit" }
);
if (release.status !== 0) {
  console.error("The commit and tag were pushed, but the GitHub Release upload failed.");
  console.error(`Retry with: gh release create ${tag} "${dmg}" "${checksum}" --verify-tag --generate-notes --title "Moirasia ${version}"`);
  process.exit(release.status ?? 1);
}

console.log(`Published Moirasia ${version}.`);
