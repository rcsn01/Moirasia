...
- **Feature host** — the Moirasia side that runs a product inside the suite process when the product ships a feature package. It persists while Menu Bar mode suspends and recreates the shell renderer.
...
- **Feature surface host.** `packages/desktop-shell/src/feature-surface-host.ts` exports `acquireFeatureSurface` through `@moirasia/desktop-shell/feature-surface-host`. It returns a stable, replaceable renderer target in suite mode without controlling the shell window. In standalone mode it creates the primary window from catalog facts, installs guards and appearance handling, loads the renderer in `ready()`, and owns activation and teardown. Features keep their controllers, leases, presence policies, and product/local/custom IPC; the shared native feature adapter owns the common suite native path.
- **Shell window lifecycle** — `src/main/shell-window.ts`; owns replaceable primary-window generations. Menu Bar close detaches and destroys the shell renderer without disposing feature backends. Reopening attaches a fresh renderer and restores the last valid page.
