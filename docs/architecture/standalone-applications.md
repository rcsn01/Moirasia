...
The shared `@moirasia/desktop-shell/native-feature-adapter` owns the structural native feature port and the common Bonded/Shout suite remote-IPC authorization, registration rollback, snapshot forwarding, and disposal. The root `NativeHostClient` remains the authenticated wire transport with framing, reconnect, timeouts, and revision filtering. In native mode, `MoirasiaFeatureService.app` owns the persistent Bonded, Shout, and Amove state, leases, and background work. Each product owns its main-only native payload decoder and command validation. Product/local/custom IPC remains local, and Amove retains its Electron shelf registrar.

`ShellWindowLifecycle` owns each primary-window generation, its shell IPC authorization, polling, navigation guards, and renderer loading. Closing in Menu Bar mode destroys that generation while keeping the selected feature owner alive. Native mode leaves the service and its leases under the native host. Local mode keeps Electron controllers alive. A replacement renderer fetches fresh snapshots when Moirasia reopens. Amove's shelf is a separate window: a visible shelf continues working while the shell renderer is absent, while hidden shelf windows are destroyed and recreated on demand.
...
Vox retains its existing suite-data merge and transitional runtime lease for stale Moirasia builds that embedded Vox. Bonded remains both controlled and embedded. In native suite mode, the shared feature service owns its network monitor, PF session, and runtime lease. Shout is also both controlled and embedded. Its native service owns the audio helper session, the driver's default-input role, and the restore bookkeeping, using the existing runtime lease (`Moirasia/shout-runtime.lock` in the shared application-data root). Local mode keeps the existing Electron controllers. These policies do not apply to Exithibition or Orbis.

## Shout

`apps/integrated/Shout` boosts every application's microphone by 0 to +30 dB with an optional soft limiter, delivered through a virtual `Shout Mic` input device so consumers of the system default input hear the amplified signal. It is macOS 27-only. A Swift helper (`ShoutAudioHelper`) owns the HAL IO and gain/limiter path; an AudioServerPlugIn driver bundle (`ShoutMic.driver`, built from the vendored libASPL library) publishes the virtual input. Installing the driver into `/Library/Audio/Plug-Ins/HAL` and restarting `coreaudiod` requires administrator approval from Shout's panel. In native suite mode, the feature service owns microphone permission and the helper's audio session because it performs the HAL work. The panel displays that state and sends commands through the authenticated service. While boosting with "Make Shout Mic the default input" enabled, the helper records the previous default input before switching and restores it on shutdown.
