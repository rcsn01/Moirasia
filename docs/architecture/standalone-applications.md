...
`acquireFeatureSurface` handles their suite-versus-standalone primary surface. Suite mode uses the shell's window. Standalone mode creates a product window from catalog facts. Moirasia's preload and renderer expose only the Amove, Bonded, and Shout bridges and panels.
