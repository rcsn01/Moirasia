...
const EMPTY: ControllerSnapshot = { applications: APPLICATION_IDS.map((id) => ({ id, label: id === 'exithibition' ? 'Exithibition' : id[0]!.toUpperCase() + id.slice(1), bundleId: '', installed: false, running: false })), appearances: { version: 1, revision: 0, values: { moirasia: 'system', amove: 'system', vox: 'system', exithibition: 'dark', bonded: 'system', orbis: 'system' } }, features: [] }
