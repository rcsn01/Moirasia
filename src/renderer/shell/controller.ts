...
const EMPTY: ControllerSnapshot = { applications: APPLICATION_IDS.map((id) => ({ id, label: id === 'exithibition' ? 'Exithibition' : id[0]!.toUpperCase() + id.slice(1), bundleId: '', installed: false, running: false })), appearances: { version: 1, revision: 0, values: { moirasia: 'system', amove: 'system', vox: 'system', exithibition: 'dark', bonded: 'system', orbis: 'system', yn360: 'system' } }, features: [] }
...
export function useController() {
  ...
  useEffect(() => { let active = true; void Promise.all([window.moirasia.getSnapshot(), window.moirasia.getSettings()]).then(([next, preferences]) => { if (active) { setSnapshot(next); setSettings(preferences) } }).catch((reason) => active && setError(message(reason))).finally(() => active && setLoading(false)); const offSnapshot = window.moirasia.onSnapshot((next) => active && setSnapshot(next)); const offNavigate = window.moirasia.onNavigate((next) => active && setPage(next)); void window.moirasia.reportPage?.('general'); return () => { active = false; offSnapshot(); offNavigate() } }, [])
  ...
  return { snapshot, settings, page, setPage: selectPage, loading, error,
