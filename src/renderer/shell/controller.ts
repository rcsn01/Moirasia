...
export function useController() {
  ...
  useEffect(() => { let active = true; void Promise.all([window.moirasia.getSnapshot(), window.moirasia.getSettings()]).then(([next, preferences]) => { if (active) { setSnapshot(next); setSettings(preferences) } }).catch((reason) => active && setError(message(reason))).finally(() => active && setLoading(false)); const offSnapshot = window.moirasia.onSnapshot((next) => active && setSnapshot(next)); const offNavigate = window.moirasia.onNavigate((next) => active && setPage(next)); void window.moirasia.reportPage?.('apps'); return () => { active = false; offSnapshot(); offNavigate() } }, [])
