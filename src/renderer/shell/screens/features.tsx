...
export function FeaturesScreen({ controller }: { controller: ReturnType<typeof useController> }): React.JSX.Element {
  ...
  return <section className="controller-page" aria-labelledby="features-heading">
    ...
    <div className="application-grid">{features.map((feature) => <Card key={feature.id}><CardHeader><div className="card-title-row"><CardTitle>{label(feature)}</CardTitle><Badge variant={feature.restartPending ? 'destructive' : feature.loaded ? 'secondary' : 'outline'}>{feature.restartPending ? 'Restart pending' : feature.loaded ? 'Loaded' : 'Not loaded'}</Badge></div><CardDescription>{FEATURE_COPY[feature.id].description}</CardDescription></CardHeader><CardFooter><Button size="sm" disabled={!feature.installed || !feature.loaded} onClick={() => controller.openFeature(feature.id)}>{feature.loaded ? 'Open' : 'Unavailable'}</Button>{feature.installed ? <Button size="sm" variant="outline" onClick={() => controller.uninstallFeature(feature.id)}>Uninstall</Button> : <Button size="sm" variant="outline" onClick={() => controller.installFeature(feature.id)}>Install</Button>}</CardFooter></Card>)}</div>
  </section>
}
