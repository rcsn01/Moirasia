...
export function AppsScreen({ controller }: { controller: ReturnType<typeof useController> }): React.JSX.Element {
  ...
  const inSuite = (id: ApplicationId): boolean => features.some((feature) => feature.id === id && feature.installed && feature.loaded)
  return <section className="controller-page" aria-labelledby="apps-heading"><p className="eyebrow">APPLICATIONS</p><h1 id="apps-heading">Your apps</h1><p className="lede">Open, focus, or gracefully quit each standalone application. They keep running when Moirasia closes.</p><div className="application-grid">{controller.snapshot.applications.map((application) => <Card key={application.id}><CardHeader><div className="card-title-row"><CardTitle>{application.label}</CardTitle><Badge variant={application.running ? 'secondary' : 'outline'}>{application.running ? 'Running' : 'Closed'}</Badge></div><CardDescription>{application.installed ? 'Installed' : 'Not installed'}</CardDescription>{inSuite(application.id) && <p className="feature-note">Running inside Moirasia</p>}{application.error && <p className="application-error">{application.error}</p>}</CardHeader><CardFooter><Button size="sm" disabled={!application.installed || !!application.busy} onClick={() => controller.open(application.id)}>{application.busy === 'opening' ? 'Opening…' : application.running ? 'Focus' : 'Open'}</Button><Button size="sm" variant="outline" disabled={!application.running || !!application.busy} onClick={() => controller.quit(application.id)}>{application.busy === 'quitting' ? 'Quitting…' : 'Quit'}</Button></CardFooter></Card>)}</div>
    <section className="feature-section" aria-labelledby="features-heading"><p className="eyebrow">FEATURES</p><h2 id="features-heading">Inside Moirasia</h2><p className="lede">Whole products that run inside this Moirasia process. Uninstalling keeps each product's settings; a restart fully unloads it.</p>
      ...
      <div className="application-grid">{features.map((feature) => <Card key={feature.id}><CardHeader><div className="card-title-row"><CardTitle>{label(feature)}</CardTitle><Badge variant={feature.restartPending ? 'destructive' : feature.loaded ? 'secondary' : 'outline'}>{feature.restartPending ? 'Restart pending' : feature.loaded ? 'Loaded' : 'Not loaded'}</Badge></div><CardDescription>{FEATURE_COPY[feature.id]?.description ?? 'Runs inside Moirasia.'}</CardDescription></CardHeader><CardFooter><Button size="sm" disabled={!feature.installed || !feature.loaded} onClick={() => controller.openFeature(feature.id)}>{feature.loaded ? 'Open' : 'Unavailable'}</Button>{feature.installed ? <Button size="sm" variant="outline" onClick={() => controller.uninstallFeature(feature.id)}>Uninstall</Button> : <Button size="sm" variant="outline" onClick={() => controller.installFeature(feature.id)}>Install</Button>}</CardFooter></Card>)}</div>
    </section>
  </section>
}
