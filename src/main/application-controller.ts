...
export class ApplicationController {
  ...
  async refresh(): Promise<ControllerSnapshot> {
    ...
    this.#applications = APPLICATION_IDS.map((id) => { const record = records.find((item) => item.id === id); const prior = this.#applications.find((item) => item.id === id); const busy = this.#busy.get(id); const error = this.#errors.get(id); const { label, bundleId } = applicationCatalog.get(id); return { id, label, bundleId, installed: record?.installed ?? false, running: record?.running ?? false, ...(record?.path ? { path: record.path } : {}), ...(prior?.loginItem ? { loginItem: prior.loginItem } : {}), ...(busy !== undefined ? { busy } : {}), ...(error !== undefined ? { error } : {}) } })
